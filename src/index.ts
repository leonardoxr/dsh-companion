...
interface EventStreamsLike {
  mux(request: { rpcId: string; payload: Record<string, never> }, signal: AbortSignal): AsyncIterable<RpcEnvelope>
...
function sessionLabel(ctx: CompanionContext, sessionId: string): string {
  const session = ctx.sessions.get(sessionId)
  const title = session === undefined ? undefined : ctx.sessionTitle.get(session)?.title
  if (title !== undefined && title.trim() !== '') return bounded(title, MAX_TITLE)
  const cwd = session?.header.cwd
  const folder = cwd?.split(/[\\/]/).filter(Boolean).at(-1)
  return bounded(folder ?? 'DeepSeek Harness', MAX_TITLE)
}

function includesSession(ctx: CompanionContext, config: NotificationConfig, sessionId: string): boolean {
...
function turnNotification(
  ...
  if (seq === undefined || kind === undefined) return undefined

  let notificationKind: NotificationKind
  let body: string
  switch (kind) {
    case 'completed':
      if (!config.completed) return undefined
      notificationKind = 'completed'
      body = 'The agent finished its turn.'
      break
    case 'blocked':
      if (!config.blocked) return undefined
      notificationKind = 'blocked'
      body = 'The agent is blocked and may need your attention.'
      break
    case 'error': {
      if (!config.errors) return undefined
...
function questionNotification(ctx: CompanionContext, envelope: RpcEnvelope): CompanionNotification | undefined {
  ...
  if (sessionId === undefined || first === undefined) return undefined
...
function approvalNotification(ctx: CompanionContext, frame: ApiFrame): CompanionNotification | undefined {
  const sessionId = text(frame.sessionId)
...
function encodeSse(event: PublishedNotification): string {
  return `id: ${event.cursor}\nevent: notification\ndata: ${JSON.stringify(event.notification)}\n\n`
}

function encodeReady(cursor: string): string {
  return `id: ${cursor}\nevent: ready\ndata: {"version":1}\n\n`
}
...
export function apply(ctx: CompanionContext, config: Config = { notifications: defaultNotifications }): () => void {
  ...
  const pending = new Map<string, CompanionNotification>()
  const recentErrors = new Map<string, Array<{ at: number; source: 'host' | 'turn' }>>()
  const streams = new AbortController()
  let counter = 0
  let disposed = false

  const currentCursor = (): string => `${instanceId}:${counter}`
  ...
  const publish = (notification: CompanionNotification): void => {
    const item: PublishedNotification = {
      counter: ++counter,
    ...
    for (const client of clients) write(client, encoded)
  }

  const isPairedError = (sessionId: string, source: 'host' | 'turn'): boolean => {
    const now = Date.now()
    for (const [id, seen] of recentErrors) {
      const active = seen.filter(item => now - item.at < ERROR_COALESCE_MS)
      if (active.length === 0) recentErrors.delete(id)
      else recentErrors.set(id, active)
    }
    const seen = recentErrors.get(sessionId) ?? []
    const pair = seen.findIndex(item => item.source !== source)
    if (pair >= 0) {
      seen.splice(pair, 1)
      if (seen.length === 0) recentErrors.delete(sessionId)
      else recentErrors.set(sessionId, seen)
      return true
    }
    ...
    ctx.webServer.register({
      ...
      handler(req, res) {
        ...
        const canReplay = cursorInstance === instanceId
          ...
          && (cursorCounter as number) >= oldestCounter - 1
          && (cursorCounter as number) <= counter

        const replayedPending = new Set<string>()
        if (canReplay) {
          for (const item of replay) {
            if (item.counter <= (cursorCounter as number)) continue
            if (item.notification.kind === 'question' || item.notification.kind === 'approval') {
              if (!pending.has(item.notification.key)) continue
        ...
        for (const notification of pending.values()) {
          if (replayedPending.has(notification.key)) continue
  ...
  const handleMux = (envelope: RpcEnvelope): void => {
    ...
    if (frame.type === 'session/event' && sessionId !== undefined) {
      ...
      if (event !== undefined) {
        ...
        if (notification?.kind === 'error' && isPairedError(sessionId, 'turn')) return
        if (notification !== undefined) publish(notification)
      }
      return
    }
    ...
    if (frame.type === 'question/resolved') {
      ...
      if (rpcId !== undefined) pending.delete(`question:${rpcId}`)
      return
    }
    ...
    if (frame.type === 'approval/resolved') {
      const approvalId = text(frame.approvalId)
      if (approvalId !== undefined) pending.delete(`approval:${approvalId}`)
    }
  }
  ...
  const handleHost = (envelope: RpcEnvelope): void => {
    ...
    if (isPairedError(sessionId, 'host')) return
    publish({
      version: 1,
  ...
  const pump = async (
    label: string,
    source: AsyncIterable<RpcEnvelope>,
    handler: (envelope: RpcEnvelope) => void,
  ): Promise<void> => {
    try {
      for await (const envelope of source) handler(envelope)
      if (!streams.signal.aborted) ctx.logger?.warn(`dsh-companion: ${label} notification stream ended unexpectedly`)
    } catch (error) {
      if (!streams.signal.aborted) ctx.logger?.warn(`dsh-companion: ${label} notification stream failed`, error)
    }
  }
